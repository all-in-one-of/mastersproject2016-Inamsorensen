...
//  BComponent+=(m_dt*_massFace*_weight*externalForceComponent);
  ...
  //Add previous velocity contribution
  float velocityComponent=_eVector.dot(particleVelocity);
  BComponent+=((_weight*particleMass*velocityComponent)/_massFace);

  //Add deviatoric force contribution
  BComponent+=((m_dt*_deviatoricForce)/_massFace);

  //Add external force contribution
  float externalForceComponent=_eVector.dot(m_externalForce);
  BComponent+=(m_dt*_weight*externalForceComponent);
...
Eigen::Matrix3f Grid::calcApComponent_DeviatoricVelocity_New(Particle *_particle, Eigen::Vector3f _weight_diff_column, Eigen::Vector3f _eVector)
{
  ...
  */
  ...
  //Calculate deltaR
  Eigen::Matrix3f deltaR=calculate_dR_New(deltaJF, R_deformElastic_Deviatoric, S_deformElastic_Deviatoric);
...
#pragma omp parallel for
  for (int cellIndex=0; cellIndex<m_totNoCells; cellIndex++)
  {
    ...
    if (m_cellFacesX[cellIndex]->m_state==State::Interior)
    {
      //Get mass
      float massX=m_cellFacesX[cellIndex]->m_mass;
      m_cellFacesX[cellIndex]->m_velocity=(m_Bvector_deviatoric_X(cellIndex)/massX);
    }

    if (m_cellFacesY[cellIndex]->m_state==State::Interior)
    {
      //Get mass
      float massY=m_cellFacesY[cellIndex]->m_mass;
      m_cellFacesY[cellIndex]->m_velocity=(m_Bvector_deviatoric_Y(cellIndex)/massY);
    }

    if (m_cellFacesZ[cellIndex]->m_state==State::Interior)
    {
      //Get mass
      float massZ=m_cellFacesZ[cellIndex]->m_mass;
      m_cellFacesZ[cellIndex]->m_velocity=(m_Bvector_deviatoric_Z(cellIndex)/massZ);
    }
  }
}
