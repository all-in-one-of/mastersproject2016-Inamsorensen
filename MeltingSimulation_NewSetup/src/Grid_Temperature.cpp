...
//#pragma omp parallel for
  for (int cellIndex=0; cellIndex<m_totNoCells; cellIndex++)
  {
    ...
    if (m_cellCentres[cellIndex]->m_state==State::Interior)
    {
      ...
      //Insert A elements
      calcAComponent_temperature(cellIndex, iIndex, jIndex, kIndex, A_matrix, A_matrix_test);
    }
...
void Grid::calcAComponent_temperature(int _cellIndex, int _iIndex, int _jIndex, int _kIndex, Eigen::SparseMatrix<double> &o_A, Eigen::MatrixXf &o_A_test)
{
  /* Outline
  ----------------------------------------------------------------------------------------------------------------
  Calculate constant
