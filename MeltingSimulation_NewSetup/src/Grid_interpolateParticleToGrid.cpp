...
#pragma omp parallel for
  for (int particleItr=0; particleItr<noParticles; particleItr++)
  {
    ...
    for (int kIndex=(kParticle-2); kIndex<(kParticle+3); kIndex++)
    {
      for (int jIndex=(jParticle-2); jIndex<(jParticle+3); jIndex++)
      {
        for (int iIndex=(iParticle-2); iIndex<(iParticle+3); iIndex++)
        {
...
//  float smallestMass=1000;
//  float largestMass=0.00001;
//  for (int testItr=0; testItr<m_totNoCells; testItr++)
//  {
//    if (m_cellCentres[testItr]->m_noParticlesContributing!=0)
//    {
//      float mass=m_cellCentres[testItr]->m_mass;
//      std::cout<<"Centre: "<<testItr<<": "<<mass<<"\n";
...
//    }
//  }
//  std::cout<<"Smallest: "<<smallestMass<<" Largest: "<<largestMass<<"\n";
...
//      std::cout<<"Particle : "<<particleItr<<": "<<particlePtr->getDensity()<<"\n";
    }
  }
...
//            }
    ...
    float smallestVel=100000;
    float largestVel=0.0000001;
    int storeLargest=0;
    int storeSmallest=0;
    for (int testItr=0; testItr<m_totNoCells; testItr++)
    {
      if (m_cellFacesY[testItr]->m_noParticlesContributing!=0)
      {
        float force=m_cellFacesY[testItr]->m_deviatoricForce;
        float B=m_Bvector_deviatoric_Y(testItr);
        float velocity=B;
        std::cout<<"FaceY: "<<testItr<<"   force: "<<force<<"   B: "<<B<<"   velocity: "<<velocity<<"\n";
        if (std::abs(velocity)<std::abs(smallestVel))
        {
          smallestVel=velocity;
          storeSmallest=testItr;
        }
        if (std::abs(velocity)>std::abs(largestVel))
        {
          largestVel=velocity;
          storeLargest=testItr;
        }

      }
    }
    std::cout<<"Smallest: "<<smallestVel<<" Largest: "<<largestVel<<"\n";
    std::cout<<"test\n";
                                   ...
                                   float &o_weightCentre, float &o_weightFaceX, float &o_weightFaceY, float &o_weightFaceZ)
{
  ...
  Clamp small values to zero. Limit 10^-5
  ...
  */
  ...
  //Calc weights
  float weightCentre=NxCentre_cubicBS*NyCentre_cubicBS*NzCentre_cubicBS;
  float weightFaceX=NxFaceX_cubicBS*NyFaceX_cubicBS*NzFaceX_cubicBS;
  float weightFaceY=NxFaceY_cubicBS*NyFaceY_cubicBS*NzFaceY_cubicBS;
  float weightFaceZ=NxFaceZ_cubicBS*NyFaceZ_cubicBS*NzFaceZ_cubicBS;

  //Return interpolation weights
  o_weightCentre=weightCentre;
  o_weightFaceX=weightFaceX;
  o_weightFaceY=weightFaceY;
  o_weightFaceZ=weightFaceZ;
                                        ...
                                        Eigen::Vector3f &o_weightFaceX, Eigen::Vector3f &o_weightFaceY, Eigen::Vector3f &o_weightFaceZ)
{
  ...
  */
  ...
  float Nx_cubicBS_FaceX=MathFunctions::calcCubicBSpline(faceXPosDiff(0)/m_cellSize);
  float Ny_cubicBS_FaceX=MathFunctions::calcCubicBSpline(faceXPosDiff(1)/m_cellSize);
  float Nz_cubicBS_FaceX=MathFunctions::calcCubicBSpline(faceXPosDiff(2)/m_cellSize);
  ...
  o_weightFaceX(0)=dNx_cubicBS_FaceX*Ny_cubicBS_FaceX*Nz_cubicBS_FaceX;
  o_weightFaceX(1)=dNy_cubicBS_FaceX*Nx_cubicBS_FaceX*Nz_cubicBS_FaceX;
  o_weightFaceX(2)=dNz_cubicBS_FaceX*Nx_cubicBS_FaceX*Ny_cubicBS_FaceX;

  o_weightFaceX*=(1.0/m_cellSize);

  //Face Y
  float Nx_cubicBS_FaceY=MathFunctions::calcCubicBSpline(faceYPosDiff(0)/m_cellSize);
  float Ny_cubicBS_FaceY=MathFunctions::calcCubicBSpline(faceYPosDiff(1)/m_cellSize);
  float Nz_cubicBS_FaceY=MathFunctions::calcCubicBSpline(faceYPosDiff(2)/m_cellSize);
  ...
  o_weightFaceY(0)=dNx_cubicBS_FaceY*Ny_cubicBS_FaceY*Nz_cubicBS_FaceY;
  o_weightFaceY(1)=dNy_cubicBS_FaceY*Nx_cubicBS_FaceY*Nz_cubicBS_FaceY;
  o_weightFaceY(2)=dNz_cubicBS_FaceY*Nx_cubicBS_FaceY*Ny_cubicBS_FaceY;

  o_weightFaceY*=(1.0/m_cellSize);

  //Face Z
  float Nx_cubicBS_FaceZ=MathFunctions::calcCubicBSpline(faceZPosDiff(0)/m_cellSize);
  float Ny_cubicBS_FaceZ=MathFunctions::calcCubicBSpline(faceZPosDiff(1)/m_cellSize);
  float Nz_cubicBS_FaceZ=MathFunctions::calcCubicBSpline(faceZPosDiff(2)/m_cellSize);
  ...
  o_weightFaceZ(0)=dNx_cubicBS_FaceZ*Ny_cubicBS_FaceZ*Nz_cubicBS_FaceZ;
  o_weightFaceZ(1)=dNy_cubicBS_FaceZ*Nx_cubicBS_FaceZ*Nz_cubicBS_FaceZ;
  o_weightFaceZ(2)=dNz_cubicBS_FaceZ*Nx_cubicBS_FaceZ*Ny_cubicBS_FaceZ;
...
//    printf("The parallel region is executed by thread %d\n", omp_get_thread_num());
    ...
    if (m_cellFacesX[cellIndex]->m_state!=State::Colliding)
    {
      ...
      //Set face X to empty as well
      if (noParticlesInCellFaceX_1<=m_noParticlesThreshold)
      {
        m_cellFacesX[cellIndex]->m_state=State::Empty;
      }

      //Set face Y to empty as well
      if (noParticlesInCellFaceY_1<=m_noParticlesThreshold && m_cellFacesY[cellIndex]->m_state!=State::Colliding)
      {
        m_cellFacesY[cellIndex]->m_state=State::Empty;
      }

      //Set face Z to empty as well
      if (noParticlesInCellFaceZ_1<=m_noParticlesThreshold && m_cellFacesZ[cellIndex]->m_state!=State::Colliding)
      {
