...
float MathFunctions::calcCubicBSpline_Diff(float _x)
{
  ...
  --------------------------------------------------------------------------------------------------
  N(x) = (3/2)*(|x|^2)*signFunction(x) - 2x*signFunction(x)                       if 0<=|x|<1
       = -(1/2)*(|x|^2)*signFunction(x) + 2x*signFunction(x) - 2*signFunction(x)  if 1<=|x|<2
       = 0                                                                        otherwise
  --------------------------------------------------------------------------------------------------
  */
...
float MathFunctions::calcTightQuadraticStencil(float _x)
{
  ...
  */
  ...
  if (absX<0.5)
  {
    result=(-1.0)*(pow(absX,2));
    result+=(3.0/4.0);
  }

  else if (absX>=0.5 && absX<1.5)
  {
    result=0.5*(pow(absX,2));
    result-=((3.0/2.0)*absX);
    result+=(9.0/8.0);
  }
...
float MathFunctions::calcTightQuadraticStencil_Diff(float _x)
{
  /*Tight Quadratic Stencil
  --------------------------------------------------------------------------------------------------
  N(x) = -2x*signX              if 0<=|x|<1/2
       = x*signX - 3/2*signX    if 1/2<=|x|<3/2  NB! Paper says 1<=|x|<3/2 but assuming meant 1/2
       = 0                otherwise
  --------------------------------------------------------------------------------------------------
  */
