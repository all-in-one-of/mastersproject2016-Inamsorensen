...
//     if (testItr==testItr2)
//     {
//       if (A_matrix_test(testItr, testItr2)!=0)
//       {
//         testSingular.insert(testItr,testItr2)=A_matrix_test(testItr, testItr2);
//       }
//       else
//       {
...
//  for (int testItr=0; testItr<m_totNoCells; testItr++)
//  {
//    if (m_cellCentres[testItr]->m_state==State::Interior)
//    {
//      float pressure=solution(testItr);
//      float B=B_vector(testItr);
//      std::cout<<"Cell: "<<testItr<<"   B: "<<B<<"   pressure: "<<pressure<<"\n";
//      if (std::abs(pressure)<std::abs(smallestVel))
//      {
//        smallestVel=pressure;
//        storeSmallest=testItr;
//      }
//      if (std::abs(pressure)>std::abs(largestVel))
//      {
//        largestVel=pressure;
...
//  //Get density
//  float densityX_i_1jk=m_cellFacesX[_cellIndex]->m_density;
//  float densityY_ij_1k=m_cellFacesY[_cellIndex]->m_density;
//  float densityZ_ijk_1=m_cellFacesZ[_cellIndex]->m_density;

//  //Cell index of neighbours
//  int cellIndex_i1jk=MathFunctions::getVectorIndex(_iIndex+1, _jIndex, _kIndex);
//  int cellIndex_ij1k=MathFunctions::getVectorIndex(_iIndex, _jIndex+1, _kIndex);
//  int cellIndex_ijk1=MathFunctions::getVectorIndex(_iIndex, _jIndex, _kIndex+1);

//  //Get density of neighbours
//  float densityX_i1jk=m_cellFacesX[cellIndex_i1jk]->m_density;
//  float densityY_ij1k=m_cellFacesY[cellIndex_ij1k]->m_density;
//  float densityZ_ijk1=m_cellFacesZ[cellIndex_ijk1]->m_density;

//  float sumDensity=densityX_i1jk+densityX_i_1jk+densityY_ij1k+densityY_ij_1k+densityZ_ijk1+densityZ_ijk_1;
...
//    float centralGradient=(1.0/m_cellSize)*(((densityX_i1jk*velocityX_forward)-(densityX_i_1jk*velocityX_backward))
//                                            +((densityY_ij1k*velocityY_forward)-(densityY_ij_1k*velocityY_backward))
//                                            +((densityZ_ijk1*velocityZ_forward)-(densityZ_ijk_1*velocityZ_backward)));
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_i1jk=(-1.0*(constant/densityX_i1jk));
    A_ijk+=(constant/densityX_i1jk);
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_i_1jk=(-1.0*(constant/densityX_i_1jk));
    A_ijk+=(constant/densityX_i_1jk);
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_ij1k=(-1.0*(constant/densityY_ij1k));
    A_ijk+=(constant/densityY_ij1k);
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_ij_1k=(-1.0*(constant/densityY_ij_1k));
    A_ijk+=(constant/densityY_ij_1k);
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_ijk1=(-1.0*(constant/densityZ_ijk1));
    A_ijk+=(constant/densityZ_ijk1);
...
//    A_ijk+=constant;
  ...
  case State::Interior :
  {
    A_ijk_1=(-1.0*(constant/densityZ_ijk_1));
    A_ijk+=(constant/densityZ_ijk_1);
...
//    A_ijk+=constant;
  ...
  //Add density here
//  pressureConst*=(densityX_i1jk + densityX_i_1jk + densityY_ij1k + densityY_ij_1k + densityZ_ijk1 + densityZ_ijk_1);
